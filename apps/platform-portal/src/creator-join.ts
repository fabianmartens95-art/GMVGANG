    ...
    return `
      <main class="join-page">
        <section class="join-shell join-shell--gate">
          ...
          <p>Melde dich zuerst mit deinem GMVGANG Account an. Danach kannst du dein Creator-Profil anlegen; ein vorhandener Referral-Code bleibt über den Login hinweg erhalten.</p>
          <a href="/login?next=${encodeURIComponent(nextPath)}" data-nav class="join-submit">Einloggen oder Account erstellen</a>
          <div class="join-gate__status"><span></span>COOKIE AUTH · SERVER VERIFIED</div>
        </section>
      </main>`;
  }
