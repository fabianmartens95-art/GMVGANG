      ...
      <section class="hero">
        <div class="hero__copy">
          ...
          <div class="hero__badges">
            ...
            ${statusPill("Workspace Context · ready")}
    ...
    return `
      <main class="locked-view">
        <div class="locked-view__box">
          ...
          <h1>${copy.title}</h1>
          <p>Dieser Bereich ist rollen- und tenant-basiert geschützt. Production akzeptiert ausschließlich serverseitig verifizierte Sessions und aktive Memberships.</p>
          <a href="${loginHref()}" data-nav class="button">Einloggen</a>
        </div>
      </main>`;
  }
...
function wireNavigation(): void {
  document.querySelectorAll<HTMLAnchorElement>("a[data-nav]").forEach((link) => {
    link.addEventListener("click", (event) => {
      ...
      if (workspaceId && !url.searchParams.has("workspace")) url.searchParams.set("workspace", workspaceId);
...
async function handleAuthCallback(): Promise<void> {
  ...
  const result = await completeAuthCallback(createBrowserAuthClient(authConfig), new URL(window.location.href));
  if (!result.ok) {
...
async function render(): Promise<void> {
  ...
  let body: string;
  if (route.path === "/login") {
    body = renderLogin(Boolean(authConfig), safeNextPath(params.get("next")));
    ...
    body = await protectedView(route.area, route);
  }
