...
function authFailure(error: unknown): boolean {
  ...
  return ["SESSION_EMAIL_NOT_VERIFIED", "SESSION_EXPIRED", "ACCOUNT_ACCESS_DENIED"].includes(error.message);
}
  ...
  return async function handlePlatformApi(request: IncomingMessage, response: ServerResponse): Promise<boolean> {
    ...
    if (url.pathname === "/api/session" && request.method === "GET") {
      try {
        ...
        sendJson(response, 200, session ?? { status: "anonymous", roles: [] });
      } catch (error) {
    ...
    if (url.pathname === "/api/creator/registration" && request.method === "POST") {
      try {
        ...
        await ensureCreatorPortalMembership(adminClient, session.userId);
