...
if (activeLanes.length > config.maxActiveLanes) {
  console.error(
    `active-lanes: ${activeLanes.length} active lanes exceed configured maximum ${config.maxActiveLanes}: ${activeLanes.join(", ")}`
  );
  process.exit(1);
}
