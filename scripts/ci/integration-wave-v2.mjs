...
Combine the selected compatible source PRs on one exact integration revision and promote them only after the repository required CI succeeds on that combined state.

## Scope

Generated integration branch containing exactly these selected source PR heads:
${included.map((source) => `- #${source.number} — ${source.title} — \`${source.sha}\``).join("\n")}

## Do not touch

No additional product, runtime, schema, auth, shared-core or business-logic changes beyond the selected source PR heads.

## Acceptance criteria

- [x] Every source PR passed its individual eligibility, dependency and required-check gates
- [x] Selected PRs have no blocked-path or file-overlap conflict
- [ ] Native pull-request CI succeeds on the combined integration revision
- [ ] Integration head/base remain unchanged through verification
- [ ] Source PRs close only after successful Wave promotion
