...
export async function handleTeamCreatorFunnel(
  request: Request,
  ...
  if (!anyRoleHasCapability(context.session.roles, "creators.read_all")) {
    return jsonResponse({ error: "creator_operations_access_denied" }, 403);
  }
  if (!dependencies.services.getTeamCreatorFunnel) {
    return jsonResponse({ error: "creator_operations_unavailable" }, 503);
  }

  const model = await dependencies.services.getTeamCreatorFunnel({ now });
