...
export async function handleCreatorReferralHub(
  request: Request,
  ...
  if (!anyRoleHasCapability(context.session.roles, "creator.referrals.read")) {
    return jsonResponse({ error: "creator_referrals_access_denied" }, 403);
  }
  if (!dependencies.services.getCreatorReferralHub) {
    return jsonResponse({ error: "creator_referrals_unavailable" }, 503);
  }

  const model = await dependencies.services.getCreatorReferralHub({
