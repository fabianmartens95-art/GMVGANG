...
export async function ensureCreatorPortalMembership(
  ...
  if (existing.error) throw new Error(`CREATOR_MEMBERSHIP_QUERY_FAILED:${existing.error.code ?? "unknown"}`);

  if (existing.data) {
    if (existing.data.status === "revoked") throw new Error("CREATOR_MEMBERSHIP_REVOKED");
    ...
    const updated = await client
      ...
      .eq("user_id", normalizedUserId)
      .eq("role", "creator");
    if (updated.error) throw new Error(`CREATOR_MEMBERSHIP_UPDATE_FAILED:${updated.error.code ?? "unknown"}`);
    return;
  }
  ...
  if (created.error) throw new Error(`CREATOR_MEMBERSHIP_INSERT_FAILED:${created.error.code ?? "unknown"}`);
}
