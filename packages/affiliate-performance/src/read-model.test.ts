...
describe("affiliate performance read model", () => {
  ...
  it("keeps overlapping affiliate-total product data and affiliate-video content data separate", () => {
    ...
    expect(model.slices.reduce((sum, slice) => sum + (slice.metrics.gmv ?? 0), 0)).toBe(180);
    // The model intentionally exposes separate slices; consumers must not interpret this sum as total GMV.
  });
